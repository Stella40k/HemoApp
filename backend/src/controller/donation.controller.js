...
export const confirmDonation = async (req, res) => {
  try {
    ...
    const institutionUser = await InstitutionModel.findOne({
      user: req.user._id,
    });
    const institutionId = institutionUser ? institutionUser._id : null;

    if (
      !institutionId ||
      donation.institution.toString() !== institutionId.toString()
    ) {
    ...
    //para las metricas
    const updatedUser = await DonationService.finalizeDonation(
      donationId,
      req.user._id
    );
    res.status(200).json({
